...
pitch_y.domain(d3.extent(pitch_track, function(d) { return d.y; }));
...
var pitch_x_function = function(d) { return x(d.x); }
var pitch_valueline = d3.line()
                      .x(pitch_x_function).y( function(d) { return pitch_y(d.y); });
...
pitch_viewplot.append("path")
    .attr("class", "line").data([pitch_track]).attr('d', function(d) {return pitch_valueline(d);})
   .style('stroke', 'blue');
...
pitch_vis.selectAll("path.line")
        .attr('d', function(d) {return pitch_valueline(d);});
...
function resizePitch(){
 pitch_valueline = d3.line()
      ...
      .y(function(d) { return pitch_y(d.y); });
  pitch_x_function = function(d) { return x(d.x); }
