function InitSelects(metadata) {
  ...
  var filter_divs = filters
        ...
        .enter()
        .append('div')
        .attr('id', function(d) {return 'div-'+d.key;});
   ...
   filter_divs.append("select")
        ...
        .selectAll('option')
        .data(function(d) {return ['all'].concat(d.values);})
        .enter()
        .append('option')
  ...
  var facet_divs = facets
   ...
   facet_divs.append("select")
        ...
        .on('change',InitFacets)
        .selectAll('option')
        .data(function(d) {return [{'key':'none'}].concat(metadata);})
        .enter()
        .append('option')
...
console.log('hello!');
    ...
    for (i=0;i<metadata.length;i++){
        ...
        if (filter != 'all'){
            filtered_data = filtered_data.filter(function(d){return d[metadata[i].key] == filter})
        }
    }

    var chart_table  = d3.select("#chart").append('table');
    if (row != 'none'){
        var row_data = metadata[metadata.findIndex(function(d) {return d.key == row;})].values
        var num_rows = row_data.length;
    }

    if (column != 'none'){
        var column_data = metadata[metadata.findIndex(function(d) {return d.key == column;})].values
    ...
    if (row != 'none'){
        ...
        console.log(row_data[i]);
            var row_filtered_data = filtered_data.filter(function(d) {return (d[row] == row_data[i]);})
            if (column != 'none'){
                for (j=0; j < num_cols; j++){
                    var svg_cell = row_divs.append('td').attr('id', 'cell-'+row_data[i]+"-" +column_data[j]);
                    var svg_data = row_filtered_data.filter(function(d) {return d[column]==column_data[j];})
        ...
        row_divs.append('td')
            ...
            if (column != 'none'){
                for (j=0; j < num_cols; j++){
                    ...
                    var column_filtered_data = filtered_data.filter(function(d) {return d[column]==column_data[j];})
...
vis.append("g")
       ...
       var parameter = vis.selectAll(".parameter")
                          .data(data_for_vis, function(d) { return d.discourse; })
                          .enter().append("g")
...
function InitChart(row, column) {
       ...
       var parameter = vis.selectAll(".parameter")
           ...
           parameter.append("path")
                    .attr("class", "line")
                    .filter(function(d) {return (d.row == row && d.column==column);})
                    .attr("d", function(d) { return valueline(d.values); })
