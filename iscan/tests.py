...
from iscan.models import Database, Corpus
...
def test_hierarchy():
    ...
    response = client.post(
        reverse('iscan:start_database_api'),
        {'name': 'test_database'},
        format="json")
    assert response.status_code == status.HTTP_202_ACCEPTED
    try:
        response = client.get(
            reverse('iscan:hierarchy_api', args=['test_corpus']),
            format='json'
        )
        ...
        # Clean up
        response = client.post(
            reverse('iscan:stop_database_api'),
            {'name': 'test_database'},
            format="json")
        assert response.status_code == status.HTTP_202_ACCEPTED
...
def test_query():
    ...
    query_data = {}
    response = client.post(
        reverse('iscan:query_api', args=['test_notacorpus']),
        query_data,
        format='json'
    )
    ...
    response = client.post(
        reverse('iscan:query_api', args=['test_corpus']),
        query_data,
        format='json'
    )
...
def test_create_database():
    ...
    response = client.post(
        reverse('iscan:create_database_api'),
        data,
        format="json")
    ...
    response = client.delete(
        reverse('iscan:database_api', args=[data['name']]),
        format="json")
    assert response.status_code == status.HTTP_202_ACCEPTED
...
def test_ports():
    ...
    response = client.get(
        reverse('iscan:database_ports_api', args=['test_database']),
        format="json")
...
def test_database_directory(pg_data_directory):
    ...
    response = client.get(
        reverse('iscan:database_directory_api', args=['test_database']),
        format="json")
    print(response.content)
    assert response.status_code == status.HTTP_200_OK
    r = json.loads(response.content)
...
def test_import(celery_app):
    ...
    data = {'name': 'test',
            ...
            'blocking': True}
    response = client.post(
        reverse('iscan:import_corpus_api'),
        data,
        format="json")
    print(response.content)
    assert response.status_code == status.HTTP_202_ACCEPTED
    # response = client.get(
    #    reverse('iscan:corpus_status_api',args=['test']),
    #    format="json")
    # print(reverse('iscan:corpus_status_api',args=['test']))
    # print(response.content)
...
def test_database_api():
    ...
    response = client.get(
        reverse('iscan:database_api', args=['test_database']),
        format="json")
    assert response.status_code == status.HTTP_200_OK
    r = json.loads(response.content)
    ...
    response = client.get(
        reverse('iscan:database_api', args=['test_notadatabase']),
        format="json")
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_corpus_api():
    ...
    try:
        ...
        response = client.get(
            reverse('iscan:corpus_api', args=['test_corpus']),
            format="json")
        assert response.status_code == status.HTTP_200_OK
        r = json.loads(response.content)
...
def test_enrich():
    ...
    response = client.post(
        reverse('iscan:enrichment_api', args=['test_notacorpus']),
        pause_data,
        format='json'
    )
    assert response.status_code == status.HTTP_404_NOT_FOUND
    ...
    response = client.post(
        reverse('iscan:enrichment_api', args=['test_corpus']),
        pause_data,
        format='json'
    )
    assert response.status_code == status.HTTP_400_BAD_REQUEST
...
def test_delete():
    ...
    response = client.delete(
        reverse('iscan:database_api', args=['notacorpus']),
        format="json")
    assert response.status_code == status.HTTP_404_NOT_FOUND
