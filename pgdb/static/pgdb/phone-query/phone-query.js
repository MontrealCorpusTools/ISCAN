angular.module('phoneQuery', [
    ...
    .filter('secondsToDateTime', [function () {
    ...
    }]).controller('PhoneQueryCtrl', function ($scope, Phones, Corpora, $state, $stateParams) {
        ...
        $scope.update = function () {
            Phones.all($stateParams.corpus_id, $scope.offset, $scope.ordering, $scope.searchText).then(function (res) {
                console.log(res.data);
                $scope.count = res.data.count;
                $scope.numPages = Math.ceil($scope.count / $scope.resultsPerPage);
                $scope.phones = res.data.results;
